...
@interface SubOfBsVsViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>

@end

@implementation SubOfBsVsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor whiteColor];
    self.title = _seenArray[0];
...
{
    UIImage *image = [UIImage imageNamed:_seenArray[0]];
    return (WIDTH-20)*(image.size.height / image.size.width) + 10 + [self theTextSizeAndwidth:WIDTH - 20 WithContent:_seenArray[1] AndFont:16] + HEIGHT/5;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        ...
        UILabel *textLabel = [[UILabel alloc]initWithFrame:CGRectMake(10,imageView.frame.origin.y + imageView.frame.size.height + 10, WIDTH-20, [self theTextSizeAndwidth:WIDTH - 20 WithContent:_seenArray[1] AndFont:16] + HEIGHT/5)];
        textLabel.font = [UIFont systemFontOfSize:16];
        textLabel.text = _seenArray[1];
